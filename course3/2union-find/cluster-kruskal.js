...
function clusterKruskal(graph, edgeobjarr, kclusters, debugmsg) {
    ...
    for (let [i,edge] of edgeobjarr.entries()) {
        if (nodeuf.parents.size < kclusters) { 
            console.log(`next edge cost: ${edgeobjarr[i-1].cost}`);
            break;
        }
...
if (runtype === 'test-single') {
    ...
    // let fp = 'E:/Dropbox/algorithms-course/course3/2union-find/clusteringDATA1-test01.txt';
    let fp = 'E:/Dropbox/algorithms-course/course3/2union-find/clusteringDATA1-test02.txt';
    ...
    let fp = 'E:/Dropbox/algorithms-course/course3/2union-find/clusteringDATA1.txt';
...
let kclusters = 4;
let [mstcostkruskal, nodeuf] = clusterKruskal(g.graph, edgeobjarr, kclusters, false);
